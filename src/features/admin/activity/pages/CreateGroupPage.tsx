      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-semibold text-slate-700 block mb-1">Maximum Size *</label>
          ...
          {selectedCaptain ? (
            <div className="flex items-center justify-between p-4 bg-indigo-50 border border-indigo-100 rounded-xl">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-800">{selectedCaptain.fullName}</h4>
                  <p className="text-sm text-slate-500">{selectedCaptain.regNo} | {selectedCaptain.departmentName}</p>
                </div>
              </div>
            ...
            <div className="relative">
              <label className="text-sm font-semibold text-slate-700 block mb-1">Search by Name or ID</label>
              <div className="relative">
                  ...
                  {searchResults.map((student) => (
                    <button key={student.regNo} type="button" onClick={() => { setSelectedCaptain(student); setSearchQuery(''); setSearchResults([]); }} className="w-full text-left px-4 py-3 hover:bg-slate-50 border-b border-slate-100 last:border-0 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500">{student.regNo} | {student.departmentName}</div>
                      </div>
                    </button>
                  ))}
                </div>
        ...
        <div className="flex justify-end pt-4 pb-12">
          <button type="submit" disabled={isSubmitting} className="px-8 py-3 bg-emerald-600 text-white font-bold rounded-xl shadow hover:bg-emerald-700 disabled:opacity-50 transition-colors flex items-center gap-2">
